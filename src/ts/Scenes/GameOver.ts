		...
		getDocs(citiesRef).then((snapshot) => {
			if (!snapshot.empty) {
				...
				docs .forEach((item, i) => {
					...
					if(item) {
						this.add.text(this.cameras.main.width / 2 - 130, 25 + (i * 28), `${item.id.slice(0, 15)} - ${data.score}`, {
							fontFamily: 'FontName',
							fontSize: '20px',
							color: '#1b5397'					
						});
